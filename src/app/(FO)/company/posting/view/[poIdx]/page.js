...
export default function ViewPage(props) {
    ...
    function getPosting() {
        ...
        axios.get(`/api/post/view?poIdx=${props.params.poIdx}`)
        .then((response) => {
    ...
    return (
        <div className="container mx-auto px-4 py-8">
                ...
                <div className="ml-8 w-1/5">
                    <div className="bg-gray-100 p-4 rounded-lg h-full">
                        ...
                        <div className="text-sm">
                            <p className="mb-1"><span className="font-medium">대표자명:</span> {posting && posting.company.coManagerName}</p>
                            <p className="mb-1"><span className="font-medium">사원수:</span> {posting && posting.company.coEmployee}</p>
                            <p className="mb-1"><span className="font-medium">업종:</span> {posting && posting.company.coType}</p>
                        </div>
                        <div className="mt-4 flex justify-between">
                            <button className="text-blue-500 text-sm">기업정보</button>
                            <button className="text-blue-500 text-sm">진행중인 채용정보</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <ul className="flex border-t-2 border-black ">
                    ...
                    <li className="flex-grow">
                        <button type="button"  className="w-full px-4 py-2 bg-gray-150 text-gray-700 hover:bg-gray-50 transition duration-300 border-r border-b border-gray-300 active:bg-white border-b-0">
                            <span>접수기간/방법</span>
                        </button>
                    </li>
                    <li className="flex-grow">
            ...
            <div className="w-3/4 pl-4 border-l border-gray-200">
                <div className="flex justify-between pl-4 border-b-6  border-black">
                    ...
                    {posting && posting.poSubType.includes('email') && (
                        <p className="text-gray-600">이메일: {posting && posting.email}</p>
                    )}
                    {posting && posting.poSubType.includes('fax') && (
                        <p className="text-gray-600">팩스: {posting && posting.fax}</p>
                    )}
                    {posting && posting.poSubType.includes('post') && (
                        <p className="text-gray-600">우편: {posting && posting.address}</p>
                    )}
            </div>
        </div>
