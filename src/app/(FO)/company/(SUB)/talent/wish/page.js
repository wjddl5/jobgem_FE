...
function Page() {
    ...
    const inputs = [
        { label: '메시지', name: 'ofContent', placeholder: '메시지를 입력하세요', type: 'textarea' },
    ];
                    ...
                    {jobseekers.map((item, idx) => (
                        <div key={idx} className="p-6 bg-white border border-gray-300 rounded-lg shadow-xl transition-shadow duration-300 hover:shadow-2xl hover:border-cyan-300">
                            <div className="flex items-center mb-4">
                                ...
                                <div className="ml-4">
                                    <h3 className="text-lg font-semibold">{item.jobseeker.joName} (만 {calculateAge(item.jobseeker.joBirth)}세)</h3>
