				...
				<div className='flex h-screen'>
					<div className='w-1/4 bg-gray-100 border-r border-gray-300 pr-1'>
						<div className='space-y-2'>
							{chatRooms.map((chat) => (
								<div key={chat.id} className={`p-3 rounded-lg ${selectedChat?.id === chat.id ? "bg-yellow-100" : "bg-white"} cursor-pointer`} onClick={() => selectedChatRoom(chat)}>
									<div className='flex justify-between'>
										<span className='font-semibold'>{chat.chatList[chat.chatList.length - 1].usIdx === userId ? "나" : "상대방"}</span>
										<span className='text-sm text-gray-500'>{chat.chatList[chat.chatList.length - 1]?.chDate}</span>
									</div>
							...
							<div className='flex flex-col justify-between h-full p-6'>
								<div className='flex-grow space-y-4 overflow-y-auto'>
									{chatList?.map((chat, index) => (
										<div key={index} className={`flex ${chat.usIdx === userId ? "justify-end" : "justify-start"}`}>
											<div className={`rounded-lg p-3 ${chat.usIdx === userId ? "bg-yellow-100" : "bg-gray-200"}`}>
								...
								<div className='flex gap-2 border-t border-gray-300 mt-4'>
									...
									/>
									<button className='flex-shrink-0 px-4 py-2 bg-blue-500 text-white rounded-lg' onClick={sendMessage}>
										전송
									</button>
								</div>
							</div>
