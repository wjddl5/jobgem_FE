...
export default function page(props) {
	...
	function changePage(event, value) {
		setPage(value - 1);
		router.replace(`/admin/bbs/qna/list?cPage=${value - 1}&searchType=${searchType}&searchValue=${searchValue}`, { shallow: true }); // 뒤로가기에도 원래 페이지로 갈 수 있게 URL수정
	}
			...
			<div className='bbs_header'>
				...
				<div className='bbs_toolBox' style={{ padding: '0 10px 0 10px' }}>
					...
					<div className='bbs_btn'>
						<Button variant='outlined' color='error' onClick={() => removeBbsList(chkSet)} disabled={deleteBtn}>
							삭제
						</Button>
					</div>
				</div>
